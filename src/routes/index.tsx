...
function ProfileSetup({ existing,preAuth=false,error,setError,onDone,onLogout,onAuthNeeded }: any) {
  ...
  return <div className={`profile-page ${theme==='light'?'light':''} ${finishing?'profile-finishing':''}`}>
    ...
    <div className="setup-wrap"><div className="setup-progress"><span style={{width:`${(step/7)*100}%`}}/></div><section className="setup-card"><div className="setup-reference-star" aria-hidden="true">✦</div><div className="setup-eyebrow">Profile setup</div><h1>{stepTitle}</h1>{step===1&&<p className="setup-subtitle">Tell us a bit about you</p>}
